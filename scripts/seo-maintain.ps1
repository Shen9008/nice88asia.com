...
function Resolve-SiteUrlToPath {
    ...
    # SEO maintenance: map production host URLs to local files (see sitemap.xml <loc> checks)
    $p = $UrlPath `
        -replace '^https://(www\.)?nice88asia\.com', '' `
...
function Resolve-HrefFromFile {
    ...
    if ($Href.StartsWith('.')) {
        # Strip #fragments so sports-betting.html#predictions resolves to the .html file
        $hrefNoFrag = $Href -replace '#.*$', ''
        $dir = Split-Path -Parent $HtmlPath
        try {
            $resolved = [System.IO.Path]::GetFullPath((Join-Path $dir $hrefNoFrag))
        } catch {
