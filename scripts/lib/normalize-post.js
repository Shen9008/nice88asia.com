...
function extractCoverImageUrl(strapiPost) {
  ...
  const candidates = [
    ...
    strapiPost.heroImage,
  ];
  for (const c of candidates) {
    const u = resolveMediaFieldToUrl(c);
    if (u) return u;
  }
  return '';
}

function resolveMediaFieldToUrl(val) {
  if (val == null || val === '') return '';
  if (typeof val === 'string') return absolutizeMediaUrl(val.trim());
  if (typeof val === 'object') {
    if (typeof val.url === 'string' && val.url) return absolutizeMediaUrl(val.url.trim());
    const d = val.data;
    if (d) {
      const entry = Array.isArray(d) ? d[0] : d;
      if (!entry) return '';
      const a = entry.attributes || entry;
      if (a.url) return absolutizeMediaUrl(String(a.url).trim());
      const fmt = a.formats && (a.formats.large || a.formats.medium || a.formats.small);
      if (fmt && fmt.url) return absolutizeMediaUrl(String(fmt.url).trim());
    }
  }
  return '';
}
...
function absolutizeMediaUrl(url) {
  ...
  const fromEnv = String(process.env.STRAPI_MEDIA_BASE || '')
    .trim()
    .replace(/\/$/, '');
  const fromApi = String(process.env.STRAPI_API_URL || '')
    .replace(/\/$/, '')
    .replace(/\/api$/i, '');
  const base = fromEnv || fromApi;
...
function normalizePost(strapiPost, opts = {}) {
  ...
  return {
    ...
    cover_image: strapiPost.cover_image || extractCoverImageUrl(strapiPost) || '',
